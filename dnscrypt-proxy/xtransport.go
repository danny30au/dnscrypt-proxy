...
// uniqueNormalizedIPs deduplicates IPs using byte array keys for better performance
	...
	if xTransport.h3Transport != nil && !hasAltSupport {
		...
		if !skipAltSvcParsing {
			if alt, found := resp.Header["Alt-Svc"]; found {
				...
				xTransport.altSupport.cache.Store(url.Host, AltSupportItem{port: altPort})
