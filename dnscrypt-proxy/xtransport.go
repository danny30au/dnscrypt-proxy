...
tlsClientConfig.ClientSessionCache = tls.NewLRUClientSessionCache(256)
}
...
headerPtr := xTransport.headerPool.Get().(*http.Header)
header := *headerPtr
defer func() {
for k := range header {
delete(header, k)
}
xTransport.headerPool.Put(headerPtr)
}()
