...
func findConfigFile(configFile *string) (string, error) {
	if _, err := os.Stat(*configFile); os.IsNotExist(err) {
		if exeFileName, exeErr := os.Executable(); exeErr == nil {
...
func (config *Config) loadSources(proxy *Proxy, configPath string) error {
	...
	for cfgSourceName, src := range config.SourcesConfig {
		g.Go(func() error {
			...
			source, err := config.newSourceFromConfig(proxy, cfgSourceName, &src)
			if err != nil {
				return err
			}
...
func includesName(names []string, name string) bool {
	return slices.ContainsFunc(names, func(found string) bool {
		return strings.EqualFold(found, name)
	})
}
