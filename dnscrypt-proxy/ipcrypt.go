...
// LUT to encode a byte into two hex characters in one 16‑bit write.
var hexByteLUT [256]uint16

func init() {
    for i := 0; i < 256; i++ {
        hi := hextable[i>>4]
        lo := hextable[i&0x0f]
        hexByteLUT[i] = uint16(hi)<<8 | uint16(lo)
    }
}
...
func NewIPCryptConfig(keyHex string, algorithm string) (*IPCryptConfig, error) {
    ...
    // Pre-initialize AES block for any AES-sized key so deterministic and ND modes
    // can use AES-NI on IPv6 paths without a slow-path init.
    if l := len(config.Key); l == 16 || l == 24 || l == 32 {
...
// newRNG returns a ChaCha8 instance, either from pool or freshly seeded.
func (config *IPCryptConfig) newRNG() *mrand.ChaCha8 {
    if v := config.rngPool.Get(); v != nil {
        return v.(*mrand.ChaCha8)
    }
    var seed [32]byte
    if _, err := crand.Read(seed[:]); err != nil {
        panic("failed to seed RNG: " + err.Error())
...
func (config *IPCryptConfig) putRNG(rng *mrand.ChaCha8) {
    config.rngPool.Put(rng)
}

// EncryptBatchWithOptions with adaptive parallelism and single result allocation
func (config *IPCryptConfig) EncryptBatchWithOptions(ips []netip.Addr, opts *BatchOptions) []string {
    ...
    if config == nil || config.Algorithm == AlgNone {
        for i, ip := range ips {
            results[i] = ip.String()
        }
        return results
    ...
    if useParallel {
        return config.encryptBatchParallelInto(ips, results, opts)
    }
    return config.encryptBatchSequentialInto(ips, results)
}

// Sequential batch processing with memory arena optimization
func (config *IPCryptConfig) encryptBatchSequentialInto(ips []netip.Addr, results []string) []string {
    n := len(ips)
    ...
    rng := config.newRNG()
    defer config.putRNG(rng)

    for i, ip := range ips {
        out := buf[:0]
        encrypted, err := config.appendEncryptIPWithRNG(out, ip, rng)
        if err != nil {
            results[i] = ip.String()
            continue
        }
...
// Parallel batch processing with per-goroutine arenas and RNG
func (config *IPCryptConfig) encryptBatchParallelInto(ips []netip.Addr, results []string, opts *BatchOptions) []string {
    n := len(ips)
    ...
    for w := 0; w < workerCount; w++ {
        ...
        go func(s, e int) {
            ...
            for i := s; i < e; i++ {
                out := buf[:0]
                encrypted, err := config.appendEncryptIPWithRNG(out, ips[i], rng)
                if err != nil {
...
// AppendEncryptIP - public main encryption dispatch; manages RNG pooling.
//
//go:inline
func (config *IPCryptConfig) AppendEncryptIP(dst []byte, ip netip.Addr) ([]byte, error) {
    ...
    return config.appendEncryptIPWithRNG(dst, ip, rng)
}

// appendEncryptIPWithRNG is the internal dispatch that reuses a caller-supplied RNG.
func (config *IPCryptConfig) appendEncryptIPWithRNG(dst []byte, ip netip.Addr, rng *mrand.ChaCha8) ([]byte, error) {
    ...
    case AlgNonDeterministic:
        return config.encryptNonDeterministic(dst, ip, 8, false, rng)
    case AlgNonDeterministicX:
        return config.encryptNonDeterministic(dst, ip, 16, true, rng)
...
// IPv4 deterministic encryption with manual loop unrolling and LUT hex
//
//go:inline
func (config *IPCryptConfig) encryptIPv4Deterministic(dst []byte, ip netip.Addr) []byte {
    ...
    // Fast LUT-based hex encoding without allocations
    var out [8]byte
    binary.BigEndian.PutUint16(out[0:2], hexByteLUT[state[0]])
    binary.BigEndian.PutUint16(out[2:4], hexByteLUT[state[1]])
    binary.BigEndian.PutUint16(out[4:6], hexByteLUT[state[2]])
    binary.BigEndian.PutUint16(out[6:8], hexByteLUT[state[3]])
...
// Non-deterministic encryption with optimized ChaCha8 RNG and word-wise XOR
func (config *IPCryptConfig) encryptNonDeterministic(
    dst []byte,
    ip netip.Addr,
    tweakSize int,
    isX bool,
    ...
    if ip.Is4() {
        ...
        // XOR with tweak (first 4 bytes; tweakSize >= 4)
        for i := 0; i < 4; i++ {
            block[i] ^= tweak[i]
        }
    ...
    // IPv6 path: 16 bytes, word-wise XOR for SIMD-friendly codegen
    ip16 := ip.As16()
    copy(block[:], ip16[:])

    b0 := binary.LittleEndian.Uint64(block[0:8])
    b1 := binary.LittleEndian.Uint64(block[8:16])

    switch tweakSize {
    case 8:
        t0 := binary.LittleEndian.Uint64(tweak[0:8])
        b0 ^= t0
        b1 ^= t0
    case 16:
        t0 := binary.LittleEndian.Uint64(tweak[0:8])
        t1 := binary.LittleEndian.Uint64(tweak[8:16])
        b0 ^= t0
        b1 ^= t1
    default:
        // Fallback for any unexpected tweakSize
        for i := 0; i < 16; i++ {
    ...
    binary.LittleEndian.PutUint64(block[0:8], b0)
    binary.LittleEndian.PutUint64(block[8:16], b1)
...
func (config *IPCryptConfig) encryptIPPrefixPreserving(dst []byte, ip netip.Addr) ([]byte, error) {
    ...
    // Mix prefix into seed using word-wise XOR (SIMD-friendly)
    p0 := binary.BigEndian.Uint32(bytes16[0:4])
    p1 := binary.BigEndian.Uint32(bytes16[4:8])
    ...
    binary.LittleEndian.PutUint32(seed[0:4], s0)
    binary.LittleEndian.PutUint32(seed[4:8], s1)

    // Generate mask and encrypt suffix
    rng := mrand.NewChaCha8(seed)
    mask := rng.Uint64()
    suffix := binary.BigEndian.Uint64(bytes16[8:])
    suffix ^= mask
    binary.BigEndian.PutUint64(bytes16[8:], suffix)

    return hex.AppendEncode(dst, bytes16[:]), nil
}
