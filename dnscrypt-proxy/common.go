...
func StringReverse(s string) string {
    // Quick check for ASCII to avoid rune allocation on the common path.
    isASCII := true
    for i := 0; i < len(s); i++ {
        if s[i] >= 0x80 {
            isASCII = false
...
// StringTwoFields - OPTIMIZED: Single pass with manual trimming, no extra slices.
...
func TrimAndStripInlineComments(str string) string {
    ...
    // Trim whitespace manually (ASCII fast path).
    start := 0
...
func ExtractHostAndPort(str string, defaultPort int) (host string, port int) {
    ...
    if err != nil {
        // Likely missing port or malformed. Treat entire string as host.
