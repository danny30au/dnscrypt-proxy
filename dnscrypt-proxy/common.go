...
// ReadPrefixed - OPTIMIZED: Use io.ReadFull for exact size reads, no buffer resizing
...
// StringTwoFields - OPTIMIZED: Single pass with direct whitespace checks
func StringTwoFields(str string) (string, string, bool) {
    ...
    j := i
    for j < len(str) && unicode.IsSpace(rune(str[j])) {
        j++
    }
    if j >= len(str) {
        return "", "", false
    ...
    k := len(str) - 1
    for k > j && unicode.IsSpace(rune(str[k])) {
        k--
    }
...
// StringStripSpaces - OPTIMIZED: ASCII fast path with strings.Builder
...
// TrimAndStripInlineComments - OPTIMIZED: Single pass comment strip, stdlib TrimFunc
...
// ExtractClientIPStr - OPTIMIZED: Type-switch on underlying addr type
...
// FormatLogLine - OPTIMIZED: Use strings.Builder for efficient formatting
