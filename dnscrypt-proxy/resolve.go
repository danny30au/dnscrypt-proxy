...
func NewResolver(server string, sendClientSubnet bool) *Resolver {
    ...
    if sendClientSubnet {
        // 93.184.216.0/24
        addr := netip.AddrFrom4([4]byte{93, 184, 216, 0})
        const bits uint8 = 24
...
func (r *Resolver) resolveQuery(
    ...
    if useECS && r.ecsOpt != nil {
        msg.Pseudo = append(msg.Pseudo[:0], r.ecsOpt)
    } else {
...
func resolveResolverInfo(
    ctx context.Context,
    r *Resolver,
    clientSubnet *string,
) ([]resolverInfo, error) {
    resp, err := r.resolveQuery(ctx, myResolverHost, dns.TypeTXT, true)
    if err != nil {
        return nil, err
    }

    infos := make([]resolverInfo, 0, len(resp.Answer))
    ...
    for _, answer := range resp.Answer {
        ...
        txts := answer.(*dns.TXT).Txt
        var ip string

        for _, s := range txts {
            switch {
            case strings.HasPrefix(s, "Resolver IP: "):
                ip = s[len("Resolver IP: "):]
            case strings.HasPrefix(s, "EDNS0 client subnet: "):
                if clientSubnet != nil {
                    *clientSubnet = s[len("EDNS0 client subnet: "):]
                }
            }
        }
    ...
    for i := range infos {
        ...
        go func() {
            ...
            resp, err := r.resolveQuery(ctx, rev, dns.TypePTR, false)
            if err != nil || resp == nil {
                return
            }
                ...
                if https.Priority != 0 || len(https.Target) <= 1 {
                    for _, value := range https.Value {
                        info = append(
                            info,
                            fmt.Sprintf("[%s]=[%s]",
                                svcb.KeyToString(svcb.PairToKey(value)),
                                value.String(),
                            ),
                        )
                    }
